...
class ContasTestes(test.APITestCase, AccountUtils):
    ...
    def test_authenticated_user_cant_put_change_other_accont(self):
        ...
        response = self.client.put(reverse("Account:detail-account", kwargs={"pk": account_user2.id}), data=data, format="json", HTTP_AUTHORIZATION=f"Bearer {token}")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
